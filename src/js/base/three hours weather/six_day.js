...
export default function getThreeHoursWeatherThirdDay ( parseData ) {
    ...
    const t = time[5].map( el => moment.unix( el ).format( 'HH:mm' ) );
    const threeHoursDiv = document.querySelector( `.three-hour-weather` );
    const markup = parseData.list[5].forecast.reduce( ( acc, el, index ) => acc + `<div class="three-hour-weather__more-info">${three_hours_weather( { el, time: t[index] } )}</div>`, '' );
