...
export default function getThreeHoursWeatherThirdDay ( parseData ) {
    ...
    const t = time[3].map( el => moment.unix( el ).format( 'HH:mm' ) );
    const threeHoursDiv = document.querySelector( `.three-hour-weather` );
    const markup = parseData.list[3].forecast.reduce( ( acc, el, index ) => acc + `<div class="three-hour-weather__more-info">${three_hours_weather( { el, time: t[index] } )}</div>`, '' );
