...
export default function getThreeHoursWeatherFirstDay ( parseData ) {
    document.querySelector( `.three-hour-weather` ).innerHTML = ``;
    const time = parseData.list.map( el => el.forecast.map( elem => elem.dt ) );
    const t = time[0].map( el => moment.unix( el ).format( 'HH:mm' ) );
    const threeHoursDiv = document.querySelector( `.three-hour-weather` );
    const markup = parseData.list[0].forecast.reduce( ( acc, el, index ) => acc + `<div class="three-hour-weather__more-info">${three_hours_weather( { el, time: t[index] } )}</div>`, '' );
